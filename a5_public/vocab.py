...
class VocabEntry(object):
    ...
    def to_input_tensor_char(self, sents: List[List[str]], device: torch.device) -> torch.Tensor:
        ...
        #sents_var = torch.tensor(pad_sents_char(charids, self.char_pad)).permute(1,0,2).contiguous()
        sents_var = torch.tensor(pad_sents_char(charids, self.char_pad)).permute(1,0,2)
        return sents_var
