...
class CharDecoder(nn.Module):
    ...
    def forward(self, input, dec_hidden=None):
        ...
        char_embedding = self.decoderCharEmb(input)# (length, batch_size, char_embedding_size)
        #print("char_embedding shape: ", char_embedding.shape)

        outputs, dec_hidden = self.charDecoder(char_embedding, dec_hidden)
    ...
    def train_forward(self, char_sequence, dec_hidden=None):
        ...
        s_t, (h_t, c_t) = self.forward(char_sequence[:-1,:], dec_hidden)
        #print("s_t: ", s_t.shape)
        #print("char_sequence: ", char_sequence.shape)
    ...
    def decode_greedy(self, initialStates, device, max_length=21):
        ...
        current_char = ['{' for _ in range(batch_size)]
        current_char_id = torch.tensor([self.target_vocab.char2id[c] for c in current_char], device=device).view(1, batch_size)

        dec_hidden = initialStates
        for i in range(max_length):
            _, (h_t, c_t) = self.forward(current_char_id, dec_hidden)
            #print(current_char_id.shape)
            s_t = self.char_output_projection(h_t)
            current_char_id = torch.argmax(s_t, 2)
            for j in range(batch_size):
                output_word[j] += self.target_vocab.id2char[int(current_char_id[0][j].item())]
            #print(h_t.shape)
            dec_hidden = (h_t, c_t)
        
        for i in range(batch_size):
            try: 
                 last_id = output_word[i].index('}')
                 output_word[i] = output_word[i][:last_id]
            except:
                continue
