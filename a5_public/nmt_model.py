...
class NMT(nn.Module):
    ...
    def __init__(self, word_embed_size, hidden_size, vocab, dropout_rate=0.3, no_char_decoder=False):
        ...
        self.h_projection = nn.Linear(hidden_size*2, hidden_size, bias=False)# shape(h,2h)
    ...
    def forward(self, source: List[List[str]], target: List[List[str]]) -> torch.Tensor:
        ...
        source_padded_chars = self.vocab.src.to_input_tensor_char(source, device=self.device)   # Tensor: (max_sent_len, src_len, b)
        target_padded_chars = self.vocab.tgt.to_input_tensor_char(target, device=self.device)   # Tensor: (max_sent_len, tgt_len, b)
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[
        ...
        enc_hiddens, (h_n, c_n) = self.encoder(pack_padded_sequence(X, source_lengths))
        enc_hiddens, _ = pad_packed_sequence(enc_hiddens, batch_first=True)

        
        final_encoder_hidden = torch.cat([torch.cat((h_n[0, i], h_n[1,i])).reshape(1, -1) for i in range(h_n.shape[1])], 0)
        init_decoder_hidden =  self.h_projection(final_encoder_hidden)

        final_encoder_cell = torch.cat([torch.cat((c_n[0, i], c_n[1,i])).reshape(1, -1) for i in range(c_n.shape[1])], 0)
        init_decoder_cell =  self.c_projection(final_encoder_cell)
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        dec_state = dec_init_state
        #print("ht, ct:", h_t.shape, c_t.shape)
        for Y_t in torch.split(Y, 1):
            Y_t = torch.squeeze(Y_t, 0)# (b, e)
            #('Y_t:{}, o_prev:{}'.format(Y_t.shape, o_prev.shape))
            Ybar_t = torch.cat((Y_t, o_prev), -1)# (b, e+h)
            #print("Ybar_t:", Ybar_t.shape)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        U_t = torch.cat((a_t, dec_hidden), -1)# (b, 3*h)
        V_t = self.combined_output_projection(U_t)# (b, h)
        tanh = nn.Tanh()
        O_t = self.dropout(tanh(V_t))# (b, h)
