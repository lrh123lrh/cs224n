...
def question_1f_sanity_check():
    ...
    dropout_rate = 0.5
    batch_size = 5
    x_convout = torch.rand((batch_size, word_embed_size))
    highway = Highway(word_embed_size, dropout_rate)
    
    x_word_emb = highway.forward(x_convout)
    output_expected_size = (batch_size, word_embed_size)
    assert(x_word_emb.shape == output_expected_size), "output shape is incorrect: it should be:\n {} but is:\n{}".format(output_expected_size, x_word_emb.shape)
...
def question_1g_sanity_check():
    ...
    seq_len = 7
    kernel_size = 3
    input = torch.rand(batch_size, word_embed_size, seq_len)

    cnn = CNN(word_embed_size, word_embed_size)
    conv_out = cnn(input)
    output_expected_size = (batch_size, word_embed_size, seq_len-kernel_size+1)
    assert(conv_out.shape == output_expected_size), "output shape is incorrect: it should be:\n {} but is:\n{}".format(output_expected_size, conv_out.shape)
